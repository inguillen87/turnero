    ...
    <div className="flex h-full gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden transition-all duration-300 w-full">
        ...
        <div className="px-4 py-2 border-b border-slate-100 bg-slate-50/70 flex flex-wrap items-center gap-2 text-xs">
          ...
          <button onClick={() => bulkSetRisk("HIGH")} className="px-2 py-1 rounded border border-red-200 text-red-600 hover:bg-red-50">Alto riesgo</button>
          <button onClick={() => bulkSetRisk("LOW")} className="px-2 py-1 rounded border border-emerald-200 text-emerald-700 hover:bg-emerald-50">Confiables</button>
          <button onClick={bulkDelete} className="px-2 py-1 rounded border border-slate-200 text-slate-700 hover:bg-slate-100">Eliminar</button>
          ...
          <button onClick={clearSelected} className="px-2 py-1 rounded border border-slate-200 text-slate-500 hover:bg-white">Limpiar</button>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-white z-10 shadow-sm">
              <tr className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100">
                <th className="py-3 pl-4 w-10"><button onClick={() => setSelectedIds(selectedIds.length === filteredPatients.length ? [] : filteredPatients.map((p) => p.id))} className="text-slate-400 hover:text-indigo-600">{selectedIds.length === filteredPatients.length && filteredPatients.length > 0 ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredPatients.map((p) => (
                <tr key={p.id} onClick={() => setSelectedPatient(p)} className="hover:bg-indigo-50/50 transition-colors cursor-pointer group pl-6">
                  <td className="py-4 pl-4"><button onClick={(e) => { e.stopPropagation(); toggleSelected(p.id); }} className="text-slate-400 hover:text-indigo-600">{selectedIds.includes(p.id) ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></td>
                  ...
                  <td className="py-4 text-right pr-6 relative" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => setOpenMenuId((prev) => (prev === p.id ? null : p.id))} className="text-slate-300 hover:text-indigo-600 transition-colors p-2 hover:bg-indigo-50 rounded-full"><MoreHorizontal className="w-5 h-5" /></button>
                    {openMenuId === p.id && (
      ...
      {(createOpen || editOpen) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white rounded-2xl border border-slate-200 shadow-2xl p-6 relative">
            <button onClick={() => { setCreateOpen(false); setEditOpen(false); }} className="absolute top-3 right-3 p-2 rounded-full hover:bg-slate-100"><X className="w-4 h-4" /></button>
